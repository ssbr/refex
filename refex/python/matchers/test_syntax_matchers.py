...
class ExprPatternTest(
    parameterized.TestCase,
    matcher_test_util.MatcherTestCase):
  ...
  @parameterized.parameters(
      'foo(kw=42)',
      'foo(1, 2, kw=42, *3)',
      'foo(1, 2, a=3, kw=42, *4, **5)',
  )
  def test_call_glob(self, s):
    """Tests pulling a keyword argument `kw` out of a call."""

    parsed = matcher.parse_ast(s, '<string>')
    expr = parsed.tree.body[0].value
    call_matcher = syntax_matchers.ExprPattern(
        'foo(*$..., **$..., kw=$kw, **$...)'
    )
    call_match = call_matcher.match(matcher.MatchContext(parsed), expr)
    self.assertIsNotNone(call_match)
    result = call_match.bindings['kw'].value.string
